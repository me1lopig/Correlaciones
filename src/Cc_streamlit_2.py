...
if not resultados:
    ...
    tab1, tab2 = st.tabs(["📊 Resultados y Estadísticas", "📚 Fórmulas Aplicadas"])

    with tab1:
        st.subheader("Estadísticas de la Estimación")
        col_metric1, col_metric2, col_metric3, col_metric4 = st.columns(4)
        
        df_validos = df_res[df_res['Cc'] > 0]
        
        col_metric1.metric("Fórmulas Evaluadas", len(df_validos))
        col_metric2.metric("Mediana de Cc", round(df_validos['Cc'].median(), 3))
        col_metric3.metric("Promedio de Cc", round(df_validos['Cc'].mean(), 3))
        col_metric4.metric("Desviación Estándar", round(df_validos['Cc'].std(), 3))

        st.markdown("---")
        
        # Dividir la pantalla en dos columnas para Gráfico y Tabla
        ...
        with col_grafico:
            ...
            # Ajuste de layout para quitar el título de Plotly y usar el de Streamlit
            fig.update_layout(margin=dict(l=0, r=0, t=30, b=0), height=500)
            st.plotly_chart(fig, use_container_width=True)
            
        with col_tabla:
            st.markdown("#### Tabla de Cálculos")
            # Mostramos únicamente Autor y Cc, ocultando el índice por defecto de Pandas
            st.dataframe(
                df_validos[['Autor', 'Cc']].sort_values(by="Cc", ascending=False).reset_index(drop=True), 
